...
st.set_page_config(layout="wide",page_title="PF Data Dispatcher",page_icon = icon_img)
...
def form_375(formid,userid,username,note,duedate,priority,project,
            auditscope,
            panel_tag,panel_sn,
            area, in_out,
            panel_type, sub_no,
            enclosure_mat, nwt_type,
            ga_no, ga_doc,
            cwd_no, cwd_doc,
            pcl_no, pcl_doc,
            spec_no, spec_doc,
    ...
    if formid == '1422047023': #changed
        pr_no_method = "Automatic"
        auditscope = auditscope
    elif formid == '1423224020': #changed
    ...
    elif formid == '1424084007': # changed
...
#dispatcher_eid = check_password()
# def credentials_check():

#     credentials_user = st.text_input("Username", key="username")
#     credentials_pass = st.text_input("Password",type="password", key="password")

#     if (credentials_user in users_df.user_name.tolist()
#         and credentials_pass == users_df.loc[users_df.user_name == credentials_user,'password'].iloc[0] 
#         ):
#         return credentials_user

# with st.expander("Credentials", expanded=True):
#     dispatcher_eid = credentials_check()

# if dispatcher_eid is not None:

    #credentials_pass = False
#pass_check_state, dispatcher_eid = check_password()

#if 'username' in st.session_state and check_password():
#if check_password() is not None:
    #st.session_state['usercred'] = check_password() 
    # if "username" not in st.session_state:
    #     st.session_state.username = False

    with st.sidebar.expander("ABOUT THE APP"):
        st.markdown("""
                    The app is built for easy dispatching of PF forms
                    to field personnel.
                """)
    ...
    #dispatcher_eid = st.session_state["usercred"]
    #dispatcher_eid = credentials_user
    #dispatcher_name = users_df.loc[users_df.user_name == dispatcher_eid,'full_name'].iloc[0]
    #dispatcher_id = users_df.loc[users_df.user_name == dispatcher_eid,'user_id'].iloc[0]
    #st.markdown(f"""👋 Welcome {dispatcher_name}""")

    #dispatchers_df = get_csvsource('s3-nvent-prontoforms-data/Data_sources/dispatchers.csv')
    ...
    #with st.form_submit_button('input'):
        ...
        # Panel
        elif form_id in (1422047023, 1423224020, 1424084007) and len(user_select) > 0 and len(project_select) > 0 and 'dispatcher_status' in st.session_state and st.session_state['dispatcher_status']:

            col_01, col_02, col_03, col_04, col_05, col_06 = st.columns([2,1,1,1,1,1])
            with col_01:
                notes = st.text_input('Dispatch note:',key='note_375')
            with col_02:
                auditscope = st.selectbox('Audit scope:',
                                            ('Level 1','Level 2','Level 3'),
                                            key='auditscope_375')
            with col_03:
                due_date = st.date_input('Due date',key='due_date_375')
            with col_04:
                priority = st.selectbox('Priority',('Low','Medium','High'),key='priority_selection_375')
            with col_05:
                panel_tag = st.text_input('Panel Tag Number:',key='pnl_tag_375')
            with col_06:
                panel_sn = st.text_input('Panel Serial Number:',key='panel_sn_375')

            col_11, col_12, col_13, col_14, col_15, col_16 = st.columns([1,1,1,1,1,1])
            with col_11:     
                area = st.text_input('Unit/Area:',key='area_375')
            with col_12:
                location = st.selectbox('Panel Location (In/Out)',('Indoor','Outdoor'),key='panel_loc_375')
            with col_13:
                type = st.selectbox('Panel Location (In/Out)',('Distribution','Control', 'Distribution & Control'),key='panel_type_375')
            with col_14:
                substation_no = st.text_input('Substation Number:',key='sub_375')
            with col_15:
                enclosure = st.selectbox('Enclosure Material',('Plastic','GRP', 'Metal Painted', 'Stainless Steel'),key='enclosure_375')
            with col_16:
                nwt_type = st.selectbox('Network Type',('TN','TT'),key='network_375')